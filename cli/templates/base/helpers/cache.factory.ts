...
export default class CacheClient {
    ...
    static init(forceEnv?: CacheEnvironment) {
        ...
        if (!["inmemory", "redis"].includes(env))
            throw new Error(
                "Invalid Caching Environment, expected - ['inmemory', 'redis'], received - " + env,
            );
    ...
    static async keys(keysample: string): Promise<string[]> {
        return this._clientMode === "redis"
            ? await this._redisClient.keys(keysample)
            : this._nodeClient.keys().filter(key => key.includes(keysample));
    }
    ...
    static async hget(key: string, field: string) {
        return this._clientMode === "redis"
            ? await this._redisClient.HGET(key, field)
            : (this._nodeClient.get(key + "." + field) as string) || null;
    }
