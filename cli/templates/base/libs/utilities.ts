    ...
    for (const key in obj) {
        ...
        newObj =
            obj[key] instanceof Object
                ? { ...newObj, ...flattenObject(obj[key], pfx) }
                : { ...newObj, [pfx]: obj[key] };
    }
    return newObj;
};
...
export const cleanObject = (obj: any) => {
    ...
    for (const k in obj) {
        if ((!k || !obj[k] || k === undefined) && typeof obj[k] !== "boolean" && obj[k] !== 0)
            delete obj[k];
    }
    return newObj;
};
...
export const parseFilter = (filter: string[], value: string, index = 0, filterMode = "_iregex") => {
